...
export default function IndividualRequestPage(): ReactElement {
    const { t } = useTranslation('common');
    const { query } = useRouter();

    const [step, setStep] = useState(0);

    const [adultCount, setAdultCount] = useState(Number(query.adultCount) ?? 4);
    const [childrenCount, setChildrenCount] = useState(Number(query.childrenCount));
